...
func (p *IPlayer) NewIPlayer(id uint32, name, ip, authToken, socialName string, socialId, hwIdHash, hwIdExHash uint64, position, rotation *Vector3) *IPlayer {
...
func (p *IPlayer) GetHealth() uint16 {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerHealth))
...
func (p *IPlayer) IsEnteringVehicle() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsEnteringVehicle))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
...
func (p *IPlayer) IsDead() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsDead))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsInVehicle() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsInVehicle))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsAiming() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsAiming))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsInCover() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsInCover))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsInRagdoll() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsInRagdoll))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsShooting() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsShooting))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsJumping() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsJumping))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsLeavingVehicle() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsLeavingVehicle))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) IsInMelle() bool {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerIsInMelle))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return cDataResult.BoolVal
...
func (p *IPlayer) Vehicle() *IVehicle {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerVehicle))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
...
func (p *IPlayer) GetPosition() *Vector3 {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerPosition))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
...
func (p *IPlayer) GetRotation() *Vector3 {
	ret, freeDataResultFunc := p.warpper.GetData(p.id, enums.Player, uint8(enums.PlayerRotation))
	cDataResult := entities.ConverCDataResult(ret)
	if cDataResult != nil {
		freeDataResultFunc()
		return (*Vector3)(cDataResult.Vector3Val)
...
func (p *IPlayer) GetPositionRotation() (*Vector3, *Vector3) {
...
func (p *IPlayer) Spawn(model ped_hash.ModelHash, position *Vector3) {
	p.model = model
	p.position = position
	p.warpper.SetPlayerMetaModelData(p.id, enums.PlayerSpawn, uint32(model), int64(uint64(math.Float32bits(position.X))|(uint64(math.Float32bits(position.Y))<<32)), uint64(math.Float32bits(position.Z))<<32)
}
...
func (p *IPlayer) Emit(eventName string, args ...any) {
	...
	s.AddTask(func() {
		...
		p.warpper.Emit(p.id, eventName, mvalues.Dump())
	})
	s.Run()
}

func (p *IPlayer) Despawn() {
	p.warpper.SetPlayerData(p.id, enums.PlayerDespawn, int64(0))
}

func (p *IPlayer) ClearBloodDamage() {
	p.warpper.SetPlayerData(p.id, enums.PlayerClearBloodDamage, int64(0))
}
...
func (p *IPlayer) SetEyeColor(eyeColor int16) {
	p.eyeColor = eyeColor
	p.warpper.SetPlayerData(p.id, enums.PlayerEyeColor, int64(eyeColor))
}

func (p *IPlayer) SetHairColor(hairColor uint8) {
	p.hairColor = hairColor
	p.warpper.SetPlayerData(p.id, enums.PlayerHairColor, int64(hairColor))
}

func (p *IPlayer) SetHairHighlightColor(hairHighlightColor uint8) {
	p.hairHighlightColor = hairHighlightColor
	p.warpper.SetPlayerData(p.id, enums.PlayerHairHighlightColor, int64(hairHighlightColor))
}

func (p *IPlayer) SetHealth(health uint16) {
	if health > hash_enums.MaxHealth {
		p.health = health
		p.warpper.SetPlayerData(p.id, enums.PlayerHealth, int64(health))
		return
	}
	p.health = health
	p.warpper.SetPlayerData(p.id, enums.PlayerHealth, int64(health))
...
func (p *IPlayer) SetPosition(position *Vector3) {
	p.position = position
	p.warpper.SetPlayerMetaData(p.id, enums.PlayerPosition, int64(math.Float32bits(position.X))|(int64(math.Float32bits(position.Y))<<32), uint64(math.Float32bits(position.Z))<<32)
}

func (p *IPlayer) SetDateTime(t time.Time) {
	p.time = t.UTC()
	p.warpper.SetPlayerData(p.id, enums.PlayerDateTime, t.UTC().Unix())
}

func (p *IPlayer) SetDateTimeUTC8(t time.Time) {
	p.time = t.UTC().Add(time.Hour * 8)
	p.warpper.SetPlayerData(p.id, enums.PlayerDateTime, t.UTC().Add(time.Hour*8).Unix())
}

func (p *IPlayer) SetWeather(wather weather_hash.WeatherType) {
	p.weather = wather
	p.warpper.SetPlayerData(p.id, enums.PlayerWeather, int64(wather))
}

func (p *IPlayer) SetMaxHealth(maxHealth uint16) {
	if maxHealth > hash_enums.MaxHealth {
		p.maxHealth = maxHealth
		p.warpper.SetPlayerData(p.id, enums.PlayerMaxHealth, int64(maxHealth))
		return
	}
	p.maxHealth = maxHealth
	p.warpper.SetPlayerData(p.id, enums.PlayerMaxHealth, int64(maxHealth))
...
func (p *IPlayer) SetMaxArmour(maxArmour uint16) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerMaxArmour, int64(maxArmour))
}

func (p *IPlayer) SetAmmo(weapon string, ammo uint16) {
	p.warpper.SetPlayerMetaData(p.id, enums.PlayerAmmo, int64(hash(weapon)), uint64(ammo))
}

func (p *IPlayer) SetMaxAmmo(weapon string, ammo uint16) {
	p.warpper.SetPlayerMetaData(p.id, enums.PlayerMaxAmmo, int64(hash(weapon)), uint64(ammo))
}

func (p *IPlayer) SetCurrentWeapon(currentWeapon weapon_hash.ModelHash) {
	p.currentWeapon = currentWeapon
	p.warpper.SetPlayerData(p.id, enums.PlayerCurrentWeapon, int64(currentWeapon))
}
...
func (p *IPlayer) SetCurrentWeaponByName(model string) {
	...
	if len(modelHash.String()) > 0 {
		...
		p.warpper.SetPlayerData(p.id, enums.PlayerCurrentWeapon, int64(modelHash))
	}
}

func (p *IPlayer) SetWeaponAmmo(weapon weapon_hash.ModelHash, ammo uint16) {
	p.warpper.SetPlayerMetaData(p.id, enums.PlayerWeaponAmmo, int64(weapon), uint64(ammo))
}
...
func (p *IPlayer) SetDimension(dimension int32) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerDimension, int64(dimension))
}

func (p *IPlayer) SetRotation(rotation *Vector3) {
	p.rotation = rotation
	p.warpper.SetPlayerMetaData(p.id, enums.PlayerRotation, int64(math.Float32bits(rotation.X))|(int64(math.Float32bits(rotation.Y))<<32), uint64(math.Float32bits(rotation.Z))<<32)
}
...
func (p *IPlayer) SetIntoVehicle(vehicle *IVehicle, seat uint8) {
	alt_timers.SetTimeout(time.Millisecond*30, func() {
		p.warpper.SetPlayerMetaData(p.id, enums.PlayerInVehicle, int64(vehicle.id), uint64(seat))
	})
}

func (p *IPlayer) SetArmour(armour uint16) {
	p.armour = armour
	p.warpper.SetPlayerData(p.id, enums.PlayerArmour, int64(armour))
}

func (p *IPlayer) SetPedModel(model ped_hash.ModelHash) {
	p.model = model
	p.warpper.SetPlayerData(p.id, enums.PlayerModel, int64(model))
}

func (p *IPlayer) SetPedModelByName(model string) {
	modelHash := ped_hash.ModelHash(hash(model))
	if len(modelHash.String()) > 0 {
		p.model = modelHash
		p.warpper.SetPlayerData(p.id, enums.PlayerModel, int64(modelHash))
	}
}
...
func (p *IPlayer) SetFrozen(frozen bool) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerFrozen, int64(value))
}
...
func (p *IPlayer) SetVisible(visible bool) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerVisible, int64(value))
}
...
func (p *IPlayer) SetCollision(collision bool) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerCollision, int64(value))
}
...
func (p *IPlayer) SetInvincible(invincible bool) {
	...
	p.warpper.SetPlayerData(p.id, enums.PlayerInvincible, int64(value))
}

func (p *IPlayer) ClearProps(compoent uint8) {
	p.warpper.SetPlayerData(p.id, enums.PlayerClearProps, int64(compoent))
}

func (p *IPlayer) ClearDecorations() {
	p.warpper.SetPlayerData(p.id, enums.PlayerClearDecorations, int64(0))
}

func (p *IPlayer) ClearTasks() {
	p.warpper.SetPlayerData(p.id, enums.PlayerClearTasks, int64(0))
}

func (p *IPlayer) Kick(reason string) {
	p.warpper.SetServerData(enums.KickPlayer, int64(p.GetId()), reason)
}

func (p *IPlayer) ResetHeadBlendData() {
	p.warpper.SetPlayerData(p.id, enums.PlayerResetHeadBlendData, int64(0))
}
...
func (p *IPlayer) SetHeadBlendData(shapeFirstId, shapeSecondId, shapeThirdId, skinFirstId, skinSecondId, skinThirdId uint32, shapeMix, skinMix, thirdMix float32) {
	p.warpper.SetPlayerHeadData(p.id, enums.PlayerHeadBlendData, shapeFirstId, shapeSecondId, shapeThirdId, skinFirstId, skinSecondId, skinThirdId, shapeMix, skinMix, thirdMix)
}

func (p *IPlayer) SetHeadOverlay(overlayId uint32, index uint8, opacity float32) {
	p.warpper.SetPlayerHeadData(p.id, enums.PlayerHeadOverlay, overlayId, uint32(index), uint32(0), uint32(0), uint32(0), uint32(0), opacity, float32(0), float32(0))
}

func (p *IPlayer) SetHeadOverlayColor(overlayId, colorType, colorIndex, secondColorIndex uint8) {
	p.warpper.SetPlayerHeadData(p.id, enums.PlayerHeadOverlayColor, uint32(overlayId), uint32(colorType), uint32(colorIndex), uint32(secondColorIndex), uint32(0), uint32(0), float32(0), float32(0), float32(0))
}

func (p *IPlayer) SetHeadBlendPaletteColor(id, r, g, b uint8) {
	p.warpper.SetPlayerHeadData(p.id, enums.PlayerHeadBlendPaletteColor, uint32(id), uint32(r), uint32(g), uint32(b), uint32(0), uint32(0), float32(0), float32(0), float32(0))
}
